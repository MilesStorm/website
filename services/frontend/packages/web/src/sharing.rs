...
async fn caller() -> Result<(String, String, Option<crate::rolls::RollHub>), ServerFnError> {
    ...
    let user = crate::rolls::arcane_user(&session)
        .await
...
pub async fn delete_my_dataset() -> Result<usize, ServerFnError> {
    let (user, token, hub) = caller().await?;
    let ds = crate::dataset::dataset().ok_or_else(|| ServerFnError::new("Sharing isn't available."))?;
    // Stop new samples first, then delete what exists.
    api::set_dataset_consent(&token, false, crate::dataset::CONSENT_VERSION)
        .await
        .map_err(store_error)?;
    if let Some(hub) = hub {
